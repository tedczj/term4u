...
**状态：R0 环境与失败基线刷新完成；不等于 R1–R6 验收通过。** 当前仅支持 macOS；Linux 验证环境不再要求，
Linux 专属代码与配置后续清理。构建空间已通过授权缓存清理恢复，本机 DNS、完整子进程树与 GUI/TUI 静默网络验证
已通过，见 [最终网络证据](../network-es-20260912/README.md)。旧失败记录按时间保留。
...
[2026-09-12 网络记录](../network-20260912/README.md)：TUI 已构建，系统 DNS 调用 PID 归因已校准；
GUI 620 秒与 TUI 65 秒已执行，但短命子进程覆盖和自动采集收尾不满足正式 PASS。
已提供手动 sudo 脚本，用户首跑明确受终端 TCC 完全磁盘访问权限阻塞。

最终更新：[完整 ES 网络验证 PASS](../network-es-20260912/README.md)，解除之前 TCC、进程组过滤
及采集收尾的阻塞；正式 C8 其他场景和 R2 真实旧样本仍未完成。

最新[真实测试清单重跑](../network-es-20260912/inventory-current.json)已完成：exit 101，134 个
旧模块引用等编译错误，取代此前磁盘不足的当前失败原因；完整测试清单仍待 R1/R6 收敛。
