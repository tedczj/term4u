...
> 仓库基线：`e25f9c174eb38458fb4c4f7a3edcd9260fb6bc5b`。
> 原批次关闭复核：用户本地候选 `000d17a7b2efb78e639882d803f57aba21ba7a73`（2026-09-14）。
> 该本地候选尚未包含在本次文档 PR 的远端代码中；不把它的测试结果写成远端 main 的结果。
...
“批次已关闭”表示原定范围已验收完成，不代表之后每个新增功能或最终 V1 自动通过。
关闭证据对应明确的源码/产物；以后发生回归单独登记并修复，不反复重开已完成的施工清单。
当前完成状态只更新本节，不新增独立 status、milestone、handoff 或 todo。

| ID | 当前状态 | 已有事实 | 剩余工作 |
|---|---|---|---|
| R0 | 原批次已关闭 | 本机工具、受控采集、系统 DNS/完整进程树归因已有验收；本次工具预检通过 | 后续例行预检；最终候选网络/防火墙验收仍归 R6，不是重做 R0 |
| R1 | 原批次已关闭 | 本地类型/初始化/调用链已验收；本地候选的 GUI/TUI/core check、严格 Clippy 和完整 presubmit 通过 | 保持回归，不重新接通接口 |
| R2 | 原批次已关闭 | 已有本地功能、真实旧 DB 迁移/写入/重启验收；本次本地候选通过测试与 16 份 DB 输入完整性检查 | 不新增人工复验；新右键菜单按 R3-F1 修复，最终候选完整矩阵仍归 R6 |
| GUI-shell 原增量 | 原批次已关闭 | 补全/prompt/clear/焦点的 12 项测试、本次完整 presubmit 和人工 H1/S8 均通过 | 无遗留人工确认；不重复原 H1，也不包括后来新增右键菜单的键盘交互 |
| R3 | 部分实现，未关闭 | 本地 Settings/Privacy/TUI 已有；本地候选新增右键菜单与 6 项通过测试 | 云 UI/action/URI/auth/flag/keybinding 负向验证；修复 R3-F1 菜单键盘焦点生命周期 |
| R4 | 未关闭 | 产品身份已改；Windows 尾项仍存在 | 外链 checker/self-test/presubmit 接入、允许清单、Windows 例外审计 |
| R5 | 部分实现，未关闭 | 本次本地候选实际 inventory 通过；云依赖主体删除、精确 pin/TUI features 已有 | 显式 git 来源、累计源码删除集及全闭包负证明；不把 inventory 通过当 R5 全部通过 |
| R6 / V1 | 未验收 | 原批次已关闭，本次工程/H1 证据可追溯 | 同一候选全部 §7 与 C1–C10；不得把本次 batch-review 当最终验收 manifest |
| M7 / V2 | 未完成，不在本轮执行 | 运行时身份已完成；文档入口已统一 | macOS 品牌资源、许可证材料、发布流程与首个正式产物 |
...
本次关闭复核不顺带实现 R3–R6 的剩余产品清理。

本次用户本地候选 `000d17a7` 的原始日志：app **1525 passed / 3 skipped**，核心/TUI
**703 / 2**，presubmit workspace **4765 / 20**，completer v2 **131 / 4**；三组严格 Clippy、
clang-format、WGSL、doc tests、完整 presubmit、GUI/TUI 构建/打包及签名检查均通过。
实际测试 ID：原始基线 9777、当前 4785、累计已登记消失 5089、相对基线新增 97。
12 项原 GUI-shell 和 6 项新增菜单测试在原始日志中均有 PASS；通过不代表未覆盖路径正确。
本次已核对 34 条自动命令、11 条续跑命令的日志 SHA256 与退出码，续跑引用的前次 result
逐字节一致。人工 H1（对应 S8，非 §1 的产品 H1）已由用户确认 PASS，隔离 GUI 正常退出。

本次 11 文件增量集中于本地右键菜单、6 项测试、标题文案和 3 份未挂载旧渲染源码删除。
没有改动持久化/migrations、网络守卫、Cargo/lock 或运行资源。R0/R2 已有运行时证据按原批次
保留；本次未重新进行网络抓包、完整真实 TUI 或 GUI 旧库迁移/写入/重启，不把完整性检查
冒充这些运行验证。最终 R6 仍必须对同一候选执行全矩阵。

**R3-F1（P1，新增菜单未关闭）**：`show_context_menu` 未将键盘焦点交给 Menu，关闭路径
也未统一处理焦点恢复；Menu 的 Esc/方向键/Enter 绑定依赖其焦点，Dismiss 只拦截鼠标。
因此这些按键可能继续作用于终端，Enter 可能执行已输入的命令而非选中菜单项。
这是高置信度静态控制流发现，本次没有 macOS 实机复现；现有 6 项菜单测试未断言该路径。
修复与自动回归见 §6.2。不能把整个 `000d17a7` 或新增菜单标成全量验收通过，也不能直接
将原始 runner 状态改为 BATCH_CLOSE_READY。原范围关闭与新缺陷并存，C4/C10/V1 仍未关闭。

更早 `d3fd6fdc` 的 R1/R2 批次是运行时兼容性来源，而不是本次测试数量的来源。
...
本次 [机器可读复核记录](../verification/000d17a7b2efb78e639882d803f57aba21ba7a73/batch-review.json)
索引用户提交的原始 ZIP/报告/patch 哈希、命令检查结果、产物身份、增量发现及适用边界。
原始文件保持不变且不公开终端正文/本机路径；此 JSON 不另定义设计或待办。
这些是用户提供的执行记录而非远程签名证明。执行器字节未随证据上传，报告的 runner 指纹与
本会话持有的 v3 附件不同；本次独立核验原始日志与报告一致性，不声称复现了相同执行器。
...
对新的源码/构建输入按 §7.1 和完整 presubmit 建立基线；未改变的 `000d17a7` 原批次不为关闭
记录重复跑同一轮，也不重复人工 H1。修复 R3-F1 后是新输入，运行对应自动回归及适用工程门禁。
...
本地候选右键菜单的 **R3-F1** 在本批处理：打开时 `ctx.focus(&self.context_menu)`；菜单 Close
事件和显式 CloseContextMenu 共用关闭函数。仅当菜单仍持有焦点时恢复终端/输入焦点，不能
抢走 Find 等动作已经设置的新焦点；保持输出选区和 alt-screen 输入边界。
自动测试必须从真实焦点状态验证：打开后菜单取得焦点；Down 选中可用项；Esc 关闭并恢复输入，
待执行文字不变；Enter 执行 Copy command 而非终端 ExecuteCommand；Find 转移焦点后关闭
不抢焦点；alt-screen 共用正确生命周期。不能只直接调用业务 action 或传入虚构菜单焦点链。
新增测试放独立测试文件或现有 `local_view_tests.rs`，先证明旧实现失败，再验证修复通过。
本次不增加人工确认项；如果自动回归发现真实无法覆盖的显示问题，再明确具体缺口而非重做 H1。
