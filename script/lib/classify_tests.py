#!/usr/bin/env python3
"""Classify current Rust tests against script/deletion_set.txt.

This is a heuristic audit helper, not the test-ID inventory or acceptance gate.
Use --detail for per-path counts and --out DIR for candidate lists.
The product design and acceptance contract live in docs/DESIGN.md.
"""
import os, re, sys, subprocess
from collections import Counter

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
DELSET = os.path.join(ROOT, 'script', 'deletion_set.txt')

# These references flag candidates for manual review, not proven compile failures.
CLASS_B_PATTERNS = [
...
def main():
    ...
    tests = subprocess.check_output(
        ['find', 'app', 'crates', '-name', '*_tests.rs', '-type', 'f'],
    ...
    print(f"类 B（断裂候选）      : {len(class_b)} 文件 / {lb} 行")
