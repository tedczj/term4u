...
root = Path('/Volumes/SN850X/term4u-l0-20260925/round1')
binary = root / 'term4u-tui'
cwd = root / 'fixtures' / 'tui-cwd'
zdotdir = root / 'fixtures' / 'zshdot'
cwd.mkdir(parents=True, exist_ok=True)
zdotdir.mkdir(parents=True, exist_ok=True)
(zdotdir / '.zshrc').write_text("PROMPT='L0TUI%# '\n", encoding='utf-8')
raw_path = root / 'raw' / 'cua18-pty-clean.raw'
events_path = root / 'logs' / 'cua18-pty-clean.jsonl'
profile = 'term4u-l0-20260925-round1-tui-clean'
...
with os.fdopen(os.open(raw_path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600), 'wb') as raw:
    ...
    event('pty_created', rows=40, cols=120, profile=profile,
          ...
          history_file=str(root / 'fixtures' / 'zsh-history'))
    env = os.environ.copy()
    env.update({'WARP_DATA_PROFILE': profile, 'TERM': 'xterm-256color',
                'LANG': 'en_US.UTF-8', 'LC_ALL': 'en_US.UTF-8',
                'WARP_SHELL_PATH': '/bin/zsh', 'SHELL': '/bin/zsh',
                'ZDOTDIR': str(zdotdir), 'HISTFILE': str(root / 'fixtures' / 'zsh-history')})
    ...
    event('process_started', pid=proc.pid, ppid=os.getpid(), termios_before={
        'lflag': before[3], 'iflag': before[0], 'oflag': before[1], 'cflag': before[2]})
    def collect(seconds):
    ...
    event('process_exited', exit_code=code, termios_after={
        'lflag': after[3], 'iflag': after[0], 'oflag': after[1], 'cflag': after[2]},
