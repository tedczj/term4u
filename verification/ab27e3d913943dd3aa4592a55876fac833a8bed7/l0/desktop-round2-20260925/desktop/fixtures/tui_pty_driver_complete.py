...
cwd = root / 'fixtures' / 'complete-tui-cwd'
zdotdir = root / 'fixtures' / 'complete-zshdot'
...
profile = 'term4u-l0-20260925-round2-tui-complete'
...
with os.fdopen(os.open(raw_path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600), 'wb') as raw:
    ...
    event('pty_created', rows=40, cols=120, profile=profile,
          ...
          history_file=str(root / 'fixtures' / 'complete-zsh-history'))
    env = os.environ.copy()
    env.update({'WARP_DATA_PROFILE': profile, 'TERM': 'xterm-256color',
                'LANG': 'en_US.UTF-8', 'LC_ALL': 'en_US.UTF-8',
                'WARP_SHELL_PATH': '/bin/zsh', 'SHELL': '/bin/zsh',
                'ZDOTDIR': str(zdotdir), 'HISTFILE': str(root / 'fixtures' / 'complete-zsh-history')})
    ...
    commands = [
        ...
        ('synthetic_osc_bel_host_passthrough_probe', b'python3 /Volumes/SN850X/term4u-l0-20260925/round2/fixtures/tui_protocol_probe.py'),
    ]
