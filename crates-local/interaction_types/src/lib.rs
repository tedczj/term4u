...
// Compatibility-only data model: all computer-control and capture operations are disabled.
...
pub fn experimental_list_windows() -> Result<String, String> {
    Err("window listing is unavailable in the offline build".to_string())
}
...
pub async fn post_process_recording(
    ...
    let _ = (input, entries, dimensions, source_duration, frame_rate);
    Err(RecordingError::Environment {
        reason: "recording is unavailable in the offline build".to_string(),
    })
}
