...
impl TabConfig {
    ...
    pub(crate) fn uses_autogenerated_branch_name(&self) -> bool {
        let contains = |value: &str| value.contains(AUTOGENERATED_BRANCH_NAME_PARAM);
        self.title.as_deref().is_some_and(contains)
            || self.panes.iter().any(|pane| {
                pane.directory.as_deref().is_some_and(contains)
                    || pane
                        .commands
                        .as_ref()
                        .is_some_and(|commands| commands.iter().any(|command| contains(command)))
            })
    }
