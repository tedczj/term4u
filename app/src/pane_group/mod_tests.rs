...
impl FocusDetectionView {
    fn new(pane_group: ViewHandle<PaneGroup>, ctx: &mut ViewContext<Self>) -> Self {
        ctx.subscribe_to_view(&pane_group, |me, pane_group, event, ctx| {
            let Event::OpenWorkflowModalWithCommand(_) = event else {
                return;
            };
...
fn test_focused_pane_is_synchronized_with_application_focus() {
    App::test((), |mut app| async move {
        ...
        pane_group.update(&mut app, |pane_group, ctx| {
            ...
            //
            // We use `OpenWorkflowModalWithCommand` because we can be confident that
            ...
            ctx.emit(Event::OpenWorkflowModalWithCommand(String::new()));
        });
    });
}
...
#[test]
fn test_undo_close_keeps_a_code_pane_watching_its_markdown_file() {
    ...
    App::test((), |mut app| async move {
        ...
        pane_group.update(&mut app, |panes, ctx| {
            let pane = CodePane::new(CodeSource::Finder { path: path.clone() }, None, ctx);
        ...
        app.read(|ctx| {
            assert!(
                file_view.as_ref(ctx).active_file_id_for_test(ctx).is_none(),
