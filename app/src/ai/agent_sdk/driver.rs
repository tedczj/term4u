...
/// Races `run_future` against the optional background Git credential refresh loop,
/// dropping the loop automatically when `run_future` resolves.
async fn with_git_credential_refresh<F, T>(
    run_future: F,
...
pub struct AgentDriver {
    ...
    /// Secrets available to the running agent and injected into its environment.
...
impl AgentDriver {
    ...
    pub fn run(
        ...
        ctx.spawn(
            async move {
                ...
                // the gap during environment setup before any
        ...
        async move {
            ...
            // occur before or outside a conversation (e.g. bootstrap,
    ...
    async fn run_internal(
        ...
        let (setup_events, task_id_for_refresh, ai_client_for_refresh) = async {
            ...
            // Wait for the terminal session to bootstrap before configuring the run.
            setup_events
            ...
            // Configure cloud providers after terminal bootstrap.
            setup_events
                .record_result(
        ...
        // Run the harness with a prompt, racing it against the optional background Git
        // credential refresh loop. The refresh future never resolves on its own and is dropped
        // automatically when the harness result resolves.
        match task.harness {
            HarnessKind::Oz => {
                ...
                let conversation_status = with_git_credential_refresh(
                    async move {
            ...
            HarnessKind::ThirdParty(harness) => {
                ...
                let (harness_exit_rx, runner) = setup_events
                    .record_result(SetupStep::ThirdPartyHarnessPreparation, async {
                        ...
                        .await?;
                        let runner =
                            Self::prepare_harness(&task.prompt, harness.as_ref(), &foreground)
                                .await?;
...
/// 2. Typed auth secrets. Inserted atomically: if any one env var for a typed secret is already
///    worker-injected, the entire secret is skipped.
