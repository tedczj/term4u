    ...
    fn discard(self, ctx: &mut ModelContext<UndoCloseStack>) {
        match self {
            ClosedItem::Window(data) => {
                if let Some(workspace) = window_workspace(data.window_id, ctx) {
            ...
            ClosedItem::Tab { data, .. } => Self::clean_up_pane_group(&data.pane_group, ctx),
            ClosedItem::Pane { data } => ctx.emit(UndoCloseStackEvent::DiscardPane(data.pane_id)),
        }
    }
